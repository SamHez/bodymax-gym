...
export const ReportService = {
    ...
    async generatePDF(scope) {
        ...
        let yPos = 20;
        doc.setFontSize(20);
        doc.text("BodyMax Gym - Executive Report", 14, yPos);
        ...
        doc.save(`BodyMax_Report_${scope}_${new Date().toISOString().split('T')[0]}.pdf`);
        return true;
    },
    ...
    async generateExcel(scope, returnBase64 = false) {
        ...
        XLSX.writeFile(wb, `BodyMax_Report_${scope}_${new Date().toISOString().split('T')[0]}.xlsx`);
        return true;
    },
    ...
    async generateEmailPayload(scope) {
        ...
        const htmlBody = `
            <div style="font-family: Arial, sans-serif; color: #333; max-width: 600px; margin: 0 auto; background-color: #f9f9fc; padding: 20px; border-radius: 8px;">
                <h2 style="color: #1E88E5; text-transform: uppercase;">BodyMax Gym - ${scopeLabel} Report</h2>
                ...
                <br />
                <p style="font-size: 12px; color: #888; text-align: center;">BodyMax Gym Management System</p>
            </div>
        `;
        return htmlBody;
    },
